...
Get-Childitem -Path ${SYSDRIVE}:\ -Include *.aspx, *.asmx, *.php -Recurse -Force -ErrorAction SilentlyContinue | Where-Object { $_.LastWriteTime -ge $DATE } | Get-Acl | Select-Object Owner,Path | Where-Object Owner -like *system | Out-file ${SYSDRIVE}:\CIR\filepaths.txt -Append | Copy-Item -Destination ${SYSDRIVE}:\CIR
